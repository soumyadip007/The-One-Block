...
//			}
			...
			{
				if(w.getKey().equals(msg))
				{
					
					System.out.println("--------------DTN Host Starts--------------------");
					System.out.println("Message From "+m.from);
					System.out.println("Message To "+m.to);
					
					System.out.println("This host Name "+this.name);
					...
					w.setRev(true);
					wr.set(index, w);
					System.out.println("This host Updated WR "+this.wr);
					
					String generatedFrom=(String) w.getHost();
					
					int gen=Integer.parseInt(generatedFrom);
					World world=MessageCreateEvent.getWorld();
					DTNHost dtn = world.getNodeByAddress(gen); 
...
//		cm.setHost(this.name);
//		this.cm.add(cm);
		
	//	Same as MessageCreateEvent
